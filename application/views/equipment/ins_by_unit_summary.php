...
<div id="main_content_header">
	...
	<div style="float:right; width:25px;">
		...
		<img id="refresh_icon" name="refresh_icon" src="/images/refresh.png" title="Refresh Log" style="cursor:pointer; float:right; height:20px; padding-top:5px;" onclick="insurance_group_by_item_selected('units')" />
	</div>
	<span style="font-weight:bold; float:right; margin-right:25px;"><?=count($insured_trucks)?></span>
</div>
...
<div id="scrollable_content" class="scrollable_div">
	<?php if(!empty($insured_trucks)):?>
			<?php foreach($insured_trucks as $insured_truck):?>
				...
				?>
				<div id="uc_for_unit_<?=$insured_truck["id"]?>" class="" style="">
					<?php foreach($truck_coverages as $truck_coverage):?>
						<?php $uc_i++;?>
						<?php include("application/views/equipment/insurance/ins_by_unit_row.php"); ?>
					<?php endforeach;?>
				</div>
				<div id="audit_row_<?=$insured_truck["id"]?>" style="height:40px; margin-top:5px;">
					<!-- AJAX GOES HERE !-->
				</div>
				<script>get_insurance_audit_row('<?=$insured_truck["id"]?>');</script>
			<?php endforeach;?>
	<?php else:?>
		No insured trucks
	<?php endif;?>
