<script>
	...
	function validate_save_staff()
	{
		...
		if (company_category == 'Select')
		{
			$("#company_category"+"_alert").show();
			isvalid = false;
		}
	...
	function save_company()
	{
		//-------------- AJAX TO LOAD TRUCK DETAILS -------------------
			...
			// AJAX!
			$.ajax({
				url: "<?= base_url("index.php/people/save_staff")?>", // in the quotation marks
				type: "POST",
				data: dataString,
				cache: false,
	...
	//PROMOTE TO FLEET MANAGER
	function promote_to_fm(company_id)
	{
		if(confirm('Are you sure you want to promote this guy?'))
		{
			//alert('promoted!');
			...
			// AJAX!
			$.ajax({
				url: "<?= base_url("index.php/people/promote_to_fm")?>", // in the quotation marks
				type: "POST",
				data: dataString,
				cache: false,
				context: main_content, // use a jquery object to select the result div in the view
				statusCode: {
					200: function(response){
						...
						//remember to reload the left bar stuff on success - if people list is not reloaded, you could have duplicate problems
						load_upper_list('Fleet Manager');
						load_fleet_manager_details(company_id)
					},
					404: function(){
						// Page not found
						alert('page not found');
...
<div id="main_content_header">
	...
	<?php if(user_has_permission('promote to fm')):?>
		<button class="jq_button" style="float:left; margin-left:20px; width:100px;" id="promote_to_fm" onclick="promote_to_fm('<?=$company["id"]?>')" >Promote to FM</button>
	<?php endif; ?>
	<img src="<?=base_url("images/save.png")?>" style="margin-top:4px;cursor:pointer;float:right;height:20px;" id="save_staff" onclick="validate_save_staff()"/>
	<img src="<?=base_url("images/back.png")?>" style="margin-top:4px;cursor:pointer;float:right;height:20px;margin-right:15px;" id="cancel_edit_staff" onclick="load_staff_details('<?=$company["id"]?>')"/>
...
<div id="scrollable_content"  class="scrollable_div">
	<div style="margin:20px;">
		...
		<table id="main_content_table" style="font-size:14px;">
			...
			<tr>
				<td style="width:300px;">Nick Name</td>
				<td>
			...
			<tr>
				...
				<td>
					<?php $options = array(
						...
						'Outside Vendor'  => 'Outside Vendor',
						); 
					?>
					<?php echo form_dropdown('company_category',$options,$company['category'],'id="company_category" class="main_content_dropdown"');?>
				</td>
				<td style="color:red; width:5px;">
					*
				</td>
				<td id="company_category_alert" class="alert">Category must be selected</td>
			</tr>
			<tr>
				<td>Status</td>
				<td>
