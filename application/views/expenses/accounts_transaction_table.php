...
<div style="width:1100px; margin:auto; margin-top:25px; font-family:arial;">
	...
	<?=form_open('expenses/add_smartpay_transactions',$attributes);?>
		<div style="width:1100px; margin-bottom:25px; text-align:center; font-weight:bold;">		
			<?=$report_name?> | <?=$account["account_name"]?>
		</div>
		<div style="width:400px; margin:auto; font-size:14px; font-weight:bold;">
			<button id="upload_transactions" name="upload_transactions" class="jq_button" style="width:200px; height:50px; margin:auto;" onclick="">Upload to Database</button>
		</div>
		<table style="margin-top:20px; font-size:14px;">
			...
			<?php foreach($entries as $entry): ?>
				<?php
					...
					//CREATE GUID FOR ENTRY
					$entry["guid"] = create_expense_guid($entry);
					
					$row_style = "";
					$link = "";
					$exists = false;
					if(expense_exists($entry))
					{
						$exists = true;
						$row_style = "color:red;";
					}
				...
				<tr style="<?=$row_style?>">
					<?php if(!$exists): ?>
						<input type="hidden" id="expense_issuer_<?=$i?>" name="expense_issuer_<?=$i?>" value="<?=$entry["issuer_id"]?>">
						...
						<input type="hidden" id="entry_guid_<?=$i?>" name="entry_guid_<?=$i?>" value="<?=$entry["guid"]?>">
					<?php endif; ?>
					<td style="width:150px; padding-right:20px;">
						<?=$entry["entry_datetime"]?>
					</td>
					<td style="width:150px;">
						<?=$issuer_company["company_side_bar_name"]?>
					</td>
