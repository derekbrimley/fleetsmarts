...
<table  style="table-layout:fixed; font-size:12px;">
	<tr class="" style="line-height:30px;">
		<td style="min-width:70px; max-width:70px;" VALIGN="top">
			<?=$date_text?>
		</td>
		<td style="min-width:80px; max-width:80px;" VALIGN="top">
			<?=$business_user["company_side_bar_name"]?>
		</td>
		<td style="min-width:80px; max-width:80px; padding-left:5px;" VALIGN="top">
			<?=$customer_vendor["company_side_bar_name"]?>
		</td>
		<td style="min-width:120px; max-width:120px; padding-left:5px;" VALIGN="top" onclick="event.stopImmediatePropagation();">
			<?php if(!empty($invoice["file_guid"])):?>
				<a target="_blank" href="<?=base_url("/index.php/documents/download_file")."/".$invoice["file_guid"]?>" onclick=""><?=$invoice["invoice_number"]?></a>
			<?php else:?>
				<?=$invoice["invoice_number"]?>
			<?php endif;?>
		</td>
		<td style="min-width:120px; max-width:120px; padding-left:5px;" title="<?=$invoice["invoice_category"]?>" VALIGN="top" class="ellipsis">
			<?=$invoice["invoice_category"]?>
		</td>
		<td style="min-width:270px; max-width:270px; padding-left:10px;" title="<?=$invoice["invoice_description"]?>" VALIGN="top" class="ellipsis">
		...
		<td style="min-width:85px; max-width:85px; text-align:right;" VALIGN="top">
